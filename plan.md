...
- [ ] 3. 提交完整预览代码与 README，推送功能分支，通过 PR 与远程 CI 更新主分支；不直接 push main、不绕过保护规则。
  ...
  - 第二次 CI：旧工具 Release 编译通过，新工具测试编译通过但并发执行 signal 11，无测试断言结果。最小探针改为串行执行同一完整测试集并输出崩溃报告，用于定位 AppKit/Carbon 进程级状态或运行时问题；未宣称根因已确认，不跳过测试。
  - 第三次 CI：完整串行测试、Python/结构/构建输出及 Release bundle 校验通过；macOS 14 安装锁探针失败。补输出隔离子进程诊断并前置安装安全检查，先定位，不绕过此门禁。
  - 安装根因已由子进程日志确认：macOS 14 没有 `/usr/bin/lockf`。复用事务库，缺少系统工具时用既有 Swift 工具链编译小型 Darwin flock helper；无额外下载依赖，父进程 fd 持锁和事务清理不变。本地签名 19/19、事务 6/6、source-only 通过，独立探针确认 Swift 锁阻断原生竞争者并在关闭后释放；远端旧系统全套重验中。永久门禁覆盖此兼容路径。
